...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, (T5Model, T5ForConditionalGeneration)) or isinstance(module, (T5Model, T5ForConditionalGenerationWithDecoderFusion)):
...
class T5StackFID(T5PreTrainedModel):
    ...
    def forward(
        ...
        for batch_size_id in range(input_ids.shape[0]):                
            ...
            positions = torch.nonzero(input_ids[batch_size_id] == 822)
            positions2 = torch.nonzero(input_ids[batch_size_id] == 10)
            positions = [i for i in positions if i+1 in positions2]
            for pos_id, pos in enumerate(positions):
                if pos_id == len(positions) - 1:
                    new_tensor = input_ids[batch_size_id][positions[pos_id]:]
                else:
                    new_tensor = input_ids[batch_size_id][positions[pos_id]:positions[pos_id+1]]
                new_tensor = new_tensor[torch.nonzero(new_tensor).t()]
        ...
        attention_mask = new_input_ids != 0
        ...
        for i in range(int(hidden_states.shape[0] / ratio)):
            if new_hidden_states == None:
                new_hidden_states = hidden_states[i*ratio:(i+1)*ratio].view(1, -1, hidden_states.shape[-1])
                new_attention_mask = attention_mask[i*ratio:(i+1)*ratio].view(1, -1)
            else:
